...
//事务中心参观start
    ...
    $api->group(['middleware' => ['api.auth']], function ($api) {
        ...
        $api->get('preordain/userinfo',
            'PreordainController@userinfo');//获取登录者的name
    });

    //学院可以进行的操作
    $api->group(['middleware' => ['api.auth', 'preordain.user']],
        function ($api) {
            $api->post('preordain/select', 'PreordainController@select');//学院预约
            $api->delete('preordain/select/{id}',
                'PreordainController@deleteSelect'); //学院删除预约
        });

    //管理员可以进行的操作
    $api->group(['middleware' => ['api.auth', 'preordain.admin']],
        function ($api) {
            $api->post('preordain/time', 'PreordainController@setTime');//设置时间段
            $api->put('preordain/time',
                'PreordainController@updateTime');//更新时间段（只能更新开始预约时间和结束预约时间）
        });
//事务中心参观end
...
//OA办公系统start
    ...
    //用户角色权限管理
    $api->get('oa/permissions',
        'PermissionController@getPermissions');    //获取所有权限
    $api->get('oa/roles',
        'PermissionController@getRoles');                //获取所有角色
    $api->get('oa/permission/{permission}',
        'PermissionController@getPermissionById');//根据id获取权限信息
    $api->get('oa/role/{role}',
        'PermissionController@getRoleById');              //根据id获取角色及其权限
    ...
    $api->put('oa/permission/{permission}',
    ...
    $api->get('oa/schedule/{schedule}',
        'OAController@getScheduleById');     //获取单个日程
    $api->post('oa/schedule', 'OAController@addSchedule');        //增加日程
    $api->put('oa/schedule/{schedule}',
        'OAController@updateSchedule');    //更新日程
    $api->delete('oa/schedule/{schedule}', 'OAController@deleteSchedule')
        ->middleware('api.auth');         //删除日程

    //设备管理
    $api->get('oa/equipments',
        'OAController@getEquipments');             //获取全部设备
    $api->get('oa/equipment/{equipment}',
        'OAController@getEquipmentById');               //获取单个设备信息
    $api->post('oa/equipment',
        'OAController@addEquipment');               //增加一个设备
    $api->put('oa/equipment/{equipment}',
        'OAController@updateEquipment');           //更新设备
    ...
    //设备借用记录管理
    $api->get('oa/devices',
        'OAController@getEquipmentRecords');     //获取所有设备借用记录
    ...
    $api->get('oa/workload/{workload}',
        'OAController@getWorkloadById'); //通过id获取任务量详情
    $api->put('oa/workload/{workload}', 'OAController@updateWorkload'); //更新任务量
    $api->delete('oa/workload/{workload}',
        'OAController@deleteWorkload');  //删除任务量
...
//学生服务
    ...
    $api->get('service/freshmanNotice',
        'FeatureController@freshmanNotice');  // 新生信息查询通告

//留言板
    $api->get('messageboard/getdata',
        'MessageBoardController@get_MessageBoard');
    $api->post('messageboard/insertdata',
        'MessageBoardController@insert_MessageBoard');
    $api->get('messageboard/delete/{id}/{key}',
